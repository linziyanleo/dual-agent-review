...
- [ ] **Codex Plan mode 卡 `working` 不切 `blocked`**（issue #249）—— 已修但旧版仍存在。
      ...
      规避：prompt 模板里**明确禁止 Codex 进入 plan mode**，让它直接写文件后退出；发送后 `dismiss_codex_plan_prompt.sh` 等待 Codex 进入 working 状态，若 idle 则检查 Plan-mode 提示并 dismiss、重发 Enter（最多 3 次 retry）。
